...
function hm_psrf_default_report_settings() {
    return array(
        ...
        'orderby'        => 'quantity',
        'orderdir'       => 'desc',
        'fields'         => array('product_id', 'product_sku', 'product_name', 'quantity_sold', 'product_cost', 'product_price', 'product_profit', 'gross_sales','total_profit' ),
        'limit_on'       => 0,
        'limit'          => 10,
        'include_header' => 1,
...
function hm_sbpf_page() {
    ...
    $fieldOptions = array(
		...
		'product_cost'        => esc_html__('Product Cost', 'product-sales-report-for-woocommerce'),
		'product_price'        => esc_html__('Product Price', 'product-sales-report-for-woocommerce'),
		'product_profit'        => esc_html__('Profit', 'product-sales-report-for-woocommerce'),
		'gross_sales'          => esc_html__('Gross Sales', 'product-sales-report-for-woocommerce'),
		'total_profit'        => esc_html__('Total Profit', 'product-sales-report-for-woocommerce'),
		'total_sales'        => esc_html__('Total Sales', 'product-sales-report-for-woocommerce'),
    ...
    foreach ($_POST['fields'] as $field) {
        switch ($field) {
            ...
            case 'quantity_sold':
                ...
                break;
			 case 'product_price':
                $header[] = esc_html__('Product Price', 'product-sales-report-for-woocommerce');
                break;
			 case 'product_cost':
                $header[] = esc_html__('Product Cost', 'product-sales-report-for-woocommerce');
                break;
			 case 'product_profit':
                $header[] = esc_html__('Profit', 'product-sales-report-for-woocommerce');
                break;
            case 'gross_sales':
                $header[] = esc_html__('Gross Sales', 'product-sales-report-for-woocommerce');
                break;
			 case 'total_profit':
                $header[] = esc_html__('Total Profit', 'product-sales-report-for-woocommerce');
                break;
			 case 'total_sales':
                $header[] = esc_html__('Total Sales', 'product-sales-report-for-woocommerce');
                break;
        ...
        foreach ($_POST['fields'] as $field) {
            switch ($field) {
                ...
                case 'quantity_sold':
                    ...
                    break;
				case 'product_cost':
                    $row[] = get_post_meta($product->product_id, '_wcj_purchase_price', true);
                    break;
				case 'product_price':
                    $row[] = get_post_meta($product->product_id, '_regular_price', true);
                    break;
				case 'product_profit':
					$profit = (get_post_meta($product->product_id, '_regular_price', true) - get_post_meta($product->product_id, '_wcj_purchase_price', true)) * $product->quantity ;
                    $row[] = $profit;
                    break;
				case 'total_profit':
